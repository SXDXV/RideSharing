...
/**
 * Класс, экземпляры которого заполняются полученной информацией от API в зависимости от
 * выбранных параметров в коде программы. В самом удобном и необходимом для программы
 * варианте используется result. Он приводит адрес к валидному значению. Остальные параметры
 * больше предназначены для конкретизации данных таких, как Kladr-код, Fias-код, и так далее.
 */
